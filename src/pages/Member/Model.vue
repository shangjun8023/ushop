<template>
  ...
  >
    ...
    >
      ...
      <el-form-item label="角色名称" prop="membername">
        <el-input v-model="formInfo.membername" autocomplete="off"></el-input>
      </el-form-item>
...
export default {
  ...
  data() {
    return {
      ...
      // 前面加星号，表示必填项
      rules: {
        membername: [{ required: true, message: "请输入角色名称", trigger: "blur" }],
    ...
    submit(){
      ...
      this.$refs.formInfo.validate((valid) => {
        if (valid) {
          ...
          if(this.info.isAdd){
            //--------新增-------------
          addMember(this.formInfo).then((res)=>{
            if(res.code==200){
              // 1、弹出成功的提示
               this.$message({
                  message: res.msg,
            ...
            //--------编辑------------
            editMember(this.formInfo).then((res)=>{
              if(res.code==200){
                // 1、弹出成功的提示
                this.$message({
                    message: res.msg,
