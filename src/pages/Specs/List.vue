    ...
    >
      ...
      <el-table-column prop="attrs" label="规格可选值">
        <template slot-scope="scope" width="300px">
         <el-tag class="tag" type="success" effect="dark" v-for="item in scope.row.attrs" :key="item">{{item}}</el-tag>
        </template>
      </el-table-column>
...
export default {
  ...
  computed:{
    // 1、拿到specsList里面的数据，此时是空的
    ...mapState({specsList:state=>state.specs.specsList}),
    // 获取管理员数据总条数
    ...mapState({specsTotal:state=>state.specs.specsTotal})
  },
  created() {
    // 3、在生命周期中再次调用用来触发specs.js中的axios请求后台数据
    this.getSpecsListAction(this.params)
  },
  methods: {
    // 2、通过getSpecsListAction()方法来获取数据
